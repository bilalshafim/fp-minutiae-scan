...
namespace IBscanUltimate
{
    public partial class DLL
    {
        ...
        */
        ...
        public const int IBSU_ERR_DEVICE_HIGHER_SDK_REQUIRED    	= -214;  /* Device is required to connect higher SDK version for runnging */
	    public const int IBSU_ERR_DEVICE_LOCK_INVALID_BUFF 			= -215;  /* The Lock-info Buff is not valid.*/
	    public const int IBSU_ERR_DEVICE_LOCK_INFO_EMPTY 			= -216;  /* The Lock-info Buff is empty.*/
	    public const int IBSU_ERR_DEVICE_LOCK_INFO_NOT_MATCHED 		= -217;  /* When the Customer Key to the devices is not registered.*/
	    public const int IBSU_ERR_DEVICE_LOCK_INVALID_CHECKSUM 		= -218;  /* Checksums between buffer and calculated are different. */
	    public const int IBSU_ERR_DEVICE_LOCK_INVALID_KEY 			= -219;  /* When Customer key is invalid. */
	    public const int IBSU_ERR_DEVICE_LOCK_LOCKED 				= -220;  /* When the device is locked. */
	    public const int IBSU_ERR_DEVICE_LOCK_ILLEGAL_DEVICE        = -221;  /* When the device is not valid from the license file */
	    public const int IBSU_ERR_DEVICE_LOCK_INVALID_SERIAL_FORMAT = -222;  /* When the serial number format is not valid */
        ...
        */
        ...
        public const int IBSU_WRN_ROLLING_SLIP_DETECTED             = 604;  /* When detected slip finger */
        public const int IBSU_WRN_SPOOF_INIT_FAILED                 = 605;  /* When Spoof initalize failed */
